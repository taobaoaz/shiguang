...
export const AIAnalyticsPage: React.FC = () => {
  ...
  const kpis: { label: string; value: string; tip: string; icon: React.ElementType; color: string }[] = [];

  const suggestions: { title: string; gain: string; body: string; actions?: string[] }[] = [];

  const risks: { title: string; level: string; body: string; score: number }[] = [];

  const members: { name: string; role: string; load: number; output: number }[] = [];

  const funnel: { stage: string; count: number; rate: number }[] = [];
