...
import {
  ...
  HardDrive, MonitorSmartphone, Palette, RefreshCw, Rocket, Settings2,
...
export const SettingsPage: React.FC = () => {
  ...
  const aiReady = gatewayGood && String(SHIGUANG_INTEGRATIONS.ai.lifecycleState) === 'active';
  ...
  return <div className="p-1 pb-5 grid xl:grid-cols-2 gap-4 items-start">
    ...
    <Panel>
      ...
      <div className="mt-3"><SmallButton primary onClick={() => void sync.refresh()} disabled={sync.busy}><RefreshCw className={clsx('w-3.5 h-3.5', sync.busy && 'animate-spin')} />重新检测</SmallButton></div>
    </Panel>
    ...
    <Panel>
      ...
      <div className="mt-3 flex flex-wrap gap-2"><SmallButton primary onClick={() => setAiOpen(true)}><Settings2 className="w-3.5 h-3.5" />查看接入配置</SmallButton><SmallButton onClick={() => void sync.refresh()} disabled={sync.busy}>检测底座</SmallButton></div>
      <p className="text-[10px] text-white/45 leading-5 mt-3">模型 API Key 不写入 localStorage、日志、工作状态或 COS。Provider 仍由 NodeGateway 持有。</p>
    </Panel>

    <Panel>
      <SectionTitle icon={CloudCog} title="COS 同步" meta="加密不可变版本，经 NodeGateway 双向同步" action={<StatusPill state={sync.connected ? 'good' : 'warn'}>{sync.connected ? '通道可达' : '通道离线'}</StatusPill>} />
    ...
    <LiquidModal open={aiOpen} onClose={() => setAiOpen(false)} title="AI 接入详情" subtitle="配置由 NodeGateway 持有，拾光只读取脱敏状态" icon={<Bot className="w-5 h-5" />} widthClass="max-w-2xl" footer={<div className="flex justify-end gap-2"><SmallButton onClick={() => setAiOpen(false)}>关闭</SmallButton><SmallButton primary onClick={() => void sync.refresh()} disabled={sync.busy}>重新检测</SmallButton></div>}>
      <div className="space-y-3 text-[11px]">
        {[
          ...
          { title: '2. Provider 配置', state: aiReady ? '已激活' : '待配置', good: aiReady, text: 'Provider、模型、允许域名和 API Key 均由 NodeGateway 的受控配置持有。' },
