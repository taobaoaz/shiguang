# 拾光 UI 与交互规范

本文件是当前拾光界面的视觉与交互真源。产品定位为全厂网络与信息化负责人的个人工作台。

## 1. 设计目标

- 高密度但不拥挤，打开后先看到今天要处理的事
- 所有统计只来自真实数据，缺失字段明确写待确认
- 保留原拾光的纯黑画布、Liquid Glass 面板和 emerald 强调色
- 800 至 1920 像素宽的桌面窗口均保持可操作
- 核心操作支持键盘、清晰焦点和减少动态效果偏好

## 2. 信息架构

| 导航 | 作用 |
|------|------|
| 今日工作台 | 今日已完成、待办、注意事项、文件与三端同步状态 |
| 收件箱 | 零散输入的统一整理入口 |
| 工作事项 | 任务、服务请求、故障、变更、巡检 |
| 信息化项目 | 项目清单和实际完成度 |
| 设备资产 | 网络、服务器、终端与系统台账 |
| 文件盘 | 五层任务目录、任务清单、对应文件索引与按需物化文件 |
| 工作统计 | 基于现有记录的完成率与分类统计 |
| 设置 | 外观、AI 接入、COS 接入、NodeGateway 和本地状态 |

工作事项固定分为五层：`收到工作 → 分类工作 → 正在干的 → 干完的 → 归档的`。界面使用横向阶段流，不允许通过删除绕过归档。

## 3. 视觉系统

### 画布与材质

- 画布：`#060807`
- 主面板：`#101A16` 上的半透明 Liquid Glass
- 模糊：`24px`、`40px`、`56px` 三档
- 圆角：主面板 `22px`，行项目 `16px`，操作按钮 `12px` 或胶囊
- 强调色：`#27E08A`
- 状态色：`#48C6D9` 表示处理中，`#F4B860` 表示等待，`#FF6B6B` 表示风险或逾期

### 文字

- 字体：`IBM Plex Sans`，中文回退 `Noto Sans SC`、`Microsoft YaHei`
- 页面标题：20 至 22 像素
- 首屏主标题：24 至 28 像素
- 卡片标题：12 至 14 像素
- 正文与元信息：10 至 12 像素
- 数字：等宽字体，26 像素为主要统计值

### 图标

- 统一使用 Lucide React
- 导航 15 像素，卡片 16 像素
- 图标必须有文本标签、title 或 aria-label，不使用 emoji 代替功能图标

## 4. 布局与响应式

- 常规桌面：`clamp(176px,14vw,280px) + 1fr`
- 1050 像素以下：侧栏收为 72 像素，仅显示图标和 title
- 560 像素以下：侧栏收为 60 像素，内容继续在主区纵向滚动
- 顶栏保持页面标题、全局搜索与新增事项按钮
- 页面内部使用响应式 grid，任何卡片不得因固定宽度溢出
- 主内容独立滚动，侧栏与顶栏保持稳定

## 5. 核心交互

- `Ctrl K` 聚焦全局搜索
- `N` 在非输入状态下打开新增事项
- 全局搜索覆盖事项、项目、资产和资料
- 新增事项使用统一 `LiquidModal`
- 编辑事项复用统一 `LiquidModal`，允许修改业务描述字段，但不得改变事项 ID、来源、文件引用或越级改变阶段
- 弹窗头部、滚动正文和固定操作区三段分离，`900×600` 窗口内保存与取消按钮必须始终可见
- 下拉选择使用 portal 版 `LiquidSelect`
- 无本地未提交变更时每 60 秒读取 NodeGateway 最新状态
- 用户写入通过 NodeGateway 的不可变版本提交；不得直连 COS
- 工作事项只能进入归档或按合法路径重开，不提供直接删除
- 新增文件必须选择对应任务；任务详情和文件盘均显示同一组逻辑文件引用
- 文件条目必须显示逻辑文件 ID 与驻留状态，不能把 `metadata-only` 伪装为已下载文件
- COS 设置页显示 Head 数、版本、拉取/提交时间、状态代码和冲突版本；多 Head 时阻断覆盖
- AI 与 COS 使用两个独立、可操作的设置入口；renderer 只填写白名单内的 endpoint/model 与 bucket/region，所有密钥只在独立的 Windows 本地安全窗口中输入
- 接入结果必须返回清晰的已配置、底座未安装、连接测试通过或失败状态；不能用“查看配置”冒充接入入口
- AI 最小连接测试只能手动触发，禁止后台自动消耗模型配额

## 6. 可访问性

- 所有可操作控件必须使用 button、input、textarea 等语义元素
- 弹窗具备 `role=dialog`、标题关联、Esc 关闭、焦点圈定和关闭后焦点恢复
- 下拉支持方向键、Home、End 和 Esc
- `:focus-visible` 使用 emerald 轮廓
- 遵守 `prefers-reduced-motion`
- 文本与背景保持足够对比，不靠颜色单独表达状态

## 7. 真实性边界

- 不显示假通知、假邮件、假上传、假下载和假 AI 能力
- 不接入专业网络扫描引擎时，只提供资产登记和检索
- 不具备附件能力时，只创建资料元数据条目
- SLA、平均响应时间和覆盖率等指标在真实字段齐全后再启用
- C·ONE 保持独立，只读 DailyBrief；不修改其现有能力和四个任务
- AI 只输出候选分类、下一步和注意项；完成状态必须来自人或执行 Agent 的证据事件
- COS 是持久化真源，NodeGateway 是唯一业务缓存，renderer 不保存业务状态

## 8. 代码映射

| 能力 | 路径 |
|------|------|
| 页面壳与导航 | `src/App.tsx` |
| 侧栏 | `src/components/layout/Sidebar.tsx` |
| 全局搜索 | `src/components/layout/TopBar.tsx` |
| 新增事项 | `src/components/modals/NewTaskModal.tsx` |
| 编辑事项 | `src/components/modals/EditTaskModal.tsx` |
| 八个页面 | `src/pages/WorkbenchPages.tsx` |
| 状态与五阶段转换 | `src/context/AppContext.tsx` |
| 五阶段工作流 | `src/components/workbench/WorkflowBoard.tsx` |
| NodeGateway 同步 | `src/context/ShiguangSyncContext.tsx` |
| 工作台语义 | `src/lib/workbench.ts` |
| 设计令牌与响应式 | `src/index.css` |
